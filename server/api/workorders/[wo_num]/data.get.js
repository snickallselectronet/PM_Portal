...
export default defineEventHandler(async (event) => {
  ...
  const [woRes, revRes] = await Promise.all([
    fetch(
      `${cfg.supabaseUrl}/rest/v1/work_orders?select=wo_num,raw_data,processed_data,is_edited&wo_num=eq.${encodeURIComponent(woNum)}&limit=1`,
      { headers }
    ),
    fetch(
      `${cfg.supabaseUrl}/rest/v1/work_order_revisions?wo_num=eq.${encodeURIComponent(woNum)}&field=eq.raw_data&order=changed_at.desc&limit=50`,
      { headers }
    )
  ])
